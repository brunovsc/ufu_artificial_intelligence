...
if method == 1:
    ...
    final_path, generated_states, explored_states = AStar.a_star(start_state, 3)
elif method == 4:
    final_path, generated_states, explored_states = DepthSearch8Game.depth_search(start_state)
elif method == 5:
    ...
    print("Comparison\n==========")
    print("Depth Search - Solution depth: %d, Expanded nodes: %d, Explored nodes: %d" % (len(final_path)),
                                                                                         generated_states,
                                                                                         explored_states)
...
if method != 5:
    if final_path is None:
        ...
        print("Solution depth = %d" % len(final_path))
        print("Total of states generated = %d" % generated_states)
        print("Total of states explored = %d" % explored_states)

        option = input("Printar saida? (s/n) -> ")
        if option == 's':
